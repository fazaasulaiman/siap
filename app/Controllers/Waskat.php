...
class Waskat extends BaseController
{
    ...
    {
        return view('waskat/arsip');
    }
    public function input()
    {
        return view('waskat/input');
    }
    ...
    {
        $dataTable = DataTables::use('waskat')
            ->select('users.username,waskat.id,waskat.nama,waskat.pesawat,waskat.noSurat,waskat.kewarganegaraan,waskat.gender,waskat.noPaspor,waskat.dokumen,waskat.tglSurat')
            ->join('users', 'users.id = waskat.users_id', 'INNER JOIN')
            ->addColumn('dokumen', function ($query) {
                return '<a href="../upload/waskat/' . $query->dokumen . '?' . time() . '" target="_blank"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>';
            })
    ...
    public function process()
    {
        ...
        if (!$this->form_validation->run($data, 'waskat')) {
        ...
        $dokumen->move(ROOTPATH . 'public/upload/waskat', preg_replace('/\s+/', '_', $data['nama']) . '(' . $data['noSurat'] . ').' . $dokumen->guessExtension(),true);
    ...
    {
        $data = $this->request->getPost();
        if (!$this->form_validation->run($data, 'removeWaskat')) {
        ...
        $waskat = new WaskatModel();
        if ($waskat->find($id)) {
            $dokumen = $waskat->find($id)->dokumen;
            $waskat->delete($id);
            echo json_encode(array("status" => TRUE));
            $path = ROOTPATH . 'public/upload/waskat/'.$dokumen;
            chmod($path, 0777);
            unlink($path);
            exit();
        } else {
